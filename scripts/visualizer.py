...
def createSingleFrame(i, mapData, allPoses, includePrevious):
    ...
    artists.append(plt.imshow(1-mapData, cmap=plt.cm.gray))

    ## Base position
    xBase = mapData.shape[1]//2
    yBase = 0
    linkLength = 10

    xs, ys = [xBase], [yBase]
    for i in range(0, len(curPose)):
        xs.append(xs[i] + linkLength*np.cos(2*np.pi - curPose[i]))
        ys.append(ys[i] - linkLength*np.sin(2*np.pi - curPose[i]))
    xs = np.array(xs)
    ys = np.array(ys)
...
def viz():
    ...
    parser.add_argument("--dpi", help="gif resolution", type=int, default=120, required=False)
    ...
    ani.save(output_dir + args.gifFilepath, dpi=args.dpi, writer=PillowWriter(fps=args.fps))
